...
describe('TurtleFarm', function () {
    ...
    describe("unstaking security", () => {
        it("should not allow someone else to unstake", async () => {
            const signer1 = await ethers.provider.getSigner(1);
            await expect(farm.connect(signer1).unstake(tokenId)).to.be.reverted;
        });

        describe("after unstaking", () => {
            before(async () => {
                await farm.unstake(tokenId);
            });

            it("should not allow unstaking again", async () => {
                await expect(farm.unstake(tokenId)).to.be.reverted;
            });
        });
    });
});